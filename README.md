...
*   **隐私保护**: 摄像头画面仅在本地处理，不做任何上传，且 UI 上默认隐藏。
...
*   **模式切换**: 
    ...
    *   ✨ **Scatter**: 粒子散落漫游。
    *   🔍 **Focus**: 聚焦查看挂在树上的照片。
*   **添加回忆**: 点击 `Add Memories` 上传照片，照片会挂在树上。
...
| 手势 | 动作 | 说明 |
| :--- | :--- | :--- |
| **左右挥手** | 旋转 | 像拨动虚拟地球仪一样左右移动手掌 |
| **上下移动** | 倾斜 | 控制视角俯仰 |
| **握拳 ✊** | 还原 | 切换回圣诞树模式 (Tree Mode) |
| **张开手掌 🖐** | 散落 | 切换到散落模式 (Scatter Mode) |
| **捏合 👌** | 聚焦 | 切换到聚焦模式 (Focus Mode) |
